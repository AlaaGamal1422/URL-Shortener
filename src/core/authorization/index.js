...
const floppyFilter = require('floppy-filter');
...
class Authorization {
    ...
    async filterByPermission(permission, data, path) {
        const filtered = floppyFilter.filterAll(_.get(data, path), permission.attributes);
        _.set(data, path, filtered);
    }

    // async filterByReadPermission(permission, data, path) {
